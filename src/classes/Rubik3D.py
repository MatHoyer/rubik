...
class Rubik3D:
    def __init__(self, rubik: Rubik) -> None:
        ...
        Entity(model="sphere", color=get_colors().BLACK.hex, scale=2, position=Vec3(1, -1, 1))
        self.center = Entity(model="sphere", color=get_colors().BLACK.hex, scale=1, position=Vec3(1, -1, 1))
        self.radius = .51
        self.faces = []
        self.set_faces()
    ...
    def set_rubik_struct(self):
        for x in range(3):
            for y in range(3):
                for z in range(3):
                    self.cube[x, y, z] = Entity(model="cube", color=get_colors().BLACK.hex,
